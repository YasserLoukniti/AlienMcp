 ...
 */
export function registerAttachFileTool(server: McpServer, bridge: Bridge): void {
  server.registerTool(
    'alien_attach_cv',
    {
      description:
        "Attach a file from the MCP server's local disk to a file input on the page. The server reads the file at `path`, encodes it as base64, and uploads via the alien_form_input handler (DataTransfer / native setter — works on Greenhouse, Lever, Workable, Kula, Pinpoint, Teamtailor, most custom Dropzone widgets). Use this when a form requires a CV/resume upload and you don't want to ship the binary through the LLM context.",
      inputSchema: {
        tabId: z.number().optional().describe('Tab ID (default: active tab)'),
        selector: z.string().describe(
          'CSS selector of the file input (or any descendant of a Dropzone wrapper — fall back to the inner <input type="file"> automatically).',
        ),
    ...
    async (args) => {
      ...
      if (typeof args.tabId === 'number') bridgeArgs.tabId = args.tabId;

      const result = await bridge.send('formInput', bridgeArgs) as {
        success: boolean;
